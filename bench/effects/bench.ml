...
let run_test_effects ~fs ~file ~out =
  let fn t =
    ...
    | G.Geometry.Polygon arr, fm ->
      let add i p = 
        G.Geometry.Position.(v ~lng:(lng p +. i) ~lat:(lat p +. i) ?altitude:(Option.map ((+.) i) (altitude p)) ())
      in
      let arr = Array.map (Array.map (add 0.01)) (G.Geometry.Polygon.to_positions arr) in
...
let run_bench ~fs ~clock ~n_iters =
    ...
    for _i = 0 to n_iters do 
      Result.get_ok (match fs with Some fs -> run_test_effects ~fs ~file:in_file ~out:out_file | None -> run_test ~file:in_file ~out:out_file)
    done;
...
let main ~fs ~clock =
  ...
  [ 10; 20; 50; 100; 200 ]
  |> List.iter (fun n_iters ->
      run_bench ~fs ~clock ~n_iters
    )
